import type * as EventTypes from '../event-types/paras_launchpad';
...
export default class EventsClass {
	...
	public subscribeOnTransferEvent(callback: (event: EventTypes.Transfer) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			...
			callback(handleEventReturn(_event, getEventTypeDescription('Transfer', 'paras_launchpad')) as EventTypes.Transfer);
		};
	...
	public subscribeOnApprovalEvent(callback: (event: EventTypes.Approval) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			...
			callback(handleEventReturn(_event, getEventTypeDescription('Approval', 'paras_launchpad')) as EventTypes.Approval);
		};
